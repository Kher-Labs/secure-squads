...
use crate::utils::{create_signer_from_path, send_and_confirm_transaction, transaction_details};
#[derive(Args)]
...
impl MultisigCreate {
    pub async fn execute(self) -> eyre::Result<()> {
        ...
        // make sure the initiator has no durable nonce accounts
        ...
        for member in &members {
            println!(
                "  - {}: {}",
                member.key,
                match member.permissions.mask {
                    1 => "Proposer".to_string(),
                    2 => "Voter".to_string(),
                    4 => "Executor".to_string(),
                    7 => "All".to_string(),
                    _ => format!("Unknown ({})", member.permissions.mask),
                }
            );
        }
        ...
        // print a warning if threshold is 1
        ...
        println!(
            "Generated random keypair for multisig: {}",
        ...
        println!("Derived Multisig Key: {}", multisig_key.0); // unique every time
        println!("Derived Program Config PDA: {}", program_config_pda.0); // same for one program config
        ...
        let treasury = ProgramConfig::try_deserialize(&mut program_config_data)
            .unwrap()
            .treasury;
        println!("Treasury Account: {}", treasury);
        ...
        match transaction_details(&transaction) {
            Ok(_) => println!("✅ Transaction details processed successfully!"),
            Err(e) => {
                eprintln!("❌ Error processing transaction details: {}", e);
            }
        }
